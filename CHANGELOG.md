...
- Claude source-install diagnostics, managed-path deletion guards, explicit Accessibility reset, transactional update rollback, and structured live health state.
...
- Claude status separates process liveness, live Accessibility trust, UI readiness, bridge health, and exact Session binding instead of treating one PID marker as proof of every layer.
- Claude source installation skips incompatible Node.js candidates and documents its Xcode Command Line Tools and Swift requirements.
