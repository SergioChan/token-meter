...
TMP="$(mktemp -d)"
trap 'rm -rf "$TMP"' EXIT
echo "Downloading Token Widget..."
curl -fsSL "$BASE/download/token-meter-macos.zip" -o "$TMP/token-meter.zip"
(cd "$TMP" && unzip -q token-meter.zip)
cd "$TMP/TokenMeter/token-meter"
