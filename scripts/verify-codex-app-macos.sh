...
if [ -L "$APP_PATH" ]; then
  printf 'Refusing a symlinked Codex application bundle.\n' >&2
  exit 1
fi
...
if [ "$APP_PATH" != "$CANONICAL_APP_PATH" ]; then
  printf 'CODEX_APP_PATH must be an absolute canonical path: %s\n' "$CANONICAL_APP_PATH" >&2
  exit 1
fi
...
if ! /usr/bin/codesign --verify --deep --strict "$APP_PATH"; then
  printf 'Codex application signature verification failed.\n' >&2
  exit 1
fi
SIGNATURE_INFO="$(/usr/bin/codesign -dv --verbose=4 "$APP_PATH" 2>&1)"
TEAM_ID="$(printf '%s\n' "$SIGNATURE_INFO" | /usr/bin/awk -F= '$1 == "TeamIdentifier" { print $2; exit }')"
if [ "$TEAM_ID" != "2DC432GLL2" ]; then
  printf 'Refusing a Codex application signed by unexpected Team ID %s\n' "${TEAM_ID:-<missing>}" >&2
  exit 1
fi
